...
Properties of Latte stars are remarkably similar to the distributions of stars in our Milky Way sample (reproduced as empty histograms on the middle and right panel for direct comparison).
% might add to this caption a sentence saying explicitly that the "disk-halo" identification/naming is using the identical cut in the Toomre diagram [this is implied but not stated explicitly: it's important because in some non-cosmological simulations, people put in a halo/disk "by hand" and so explicitly tag which particles are which -- that's not whats being done here so saying it clearly is important]
}
\label{fig:latte}
\end{center}
\end{figure}
...
The Toomre diagram for the sample in Latte is shown in the left panel of Figure~\ref{fig:latte}.
...
Finally, we analyze orbital properties of stars in Latte by showing the orientation of their angular momenta with respect to the $Z$ axis, $\theta_L$, in the right panel of Figure~\ref{fig:latte}.
The angular momenta of Latte disk stars are well aligned with the $Z$ axis, with $\theta_L\simeq180^\circ$, similar to disk stars in the Milky Way.
...
Stars with disk kinematics (as defined by our Toomre-diagram cut in Figure~6) are shown in red, while those identified as halo are blue.
...
In summary, Latte stars formed at late times formed in the disk, while old stars in Latte are predominantly part of the halo.
...
\caption{Average fraction of accreted stars (using our definition in Figure~7) in the solar neighborhood of the Latte simulation as a function of metallicity and current angular momentum orientation angle.
Whether or not a star was accreted does not correlat with its current orbital properties, but it correlates well with its metallicity.
...
Thus, in the inner halo, a star's metallicity is a much better indicator than its current kinematics of its origin (the probability that it was accreted).
}
\label{fig:facc}
\end{center}
\end{figure}
...
- furthermore is most effective for stars with low sig\_z, so even if stars on hot orbits formed in the inner galaxy, they wouldn't necessarily be the ones migrating outwards (vera-ciro 2016)

- but these are idealized, not cosmological
- radial migration common in simulated low mass galaxies (kareem), caused by gas inflow/outflows still forming stars -- start with initial velocity + global change of the potential due to feedback
- second effect likely negligible for the MW progenitor, even at early times (check)
- but first operating -- evident from stellar tracks in Figure~\ref{fig:dform}

If metal-rich halo stars truly originate from the inner Galaxy, then they not only illustrate an important dynamical mechanism shaping the Galaxy, but are also a unique window into star formation in the early Milky Way.
In the next section, we demonstrate how the Latte model is predictive, and propose to test this origin scenario using stellar ages.

% [IN PROGRESS]
% - kareem's paper for radial migration mechanism noted in a dwarf galaxy simulated as a part of the FIRE project
% - describe mechanism briefly
% - compare final dwarf mass to MW before the significant merger, if similar -- can infer the mechanism is similar, leave further analysis for the future
...
% - also kordopatis 2015, rave, vertical structure independent of metallicity -- radial migration equally efficient for orbits of different scale-heights
% - confirms picture of solway 2012, opposite of vera-ciro 2014, who saw most efficient migration for low $\sigma_z$
% - loebman 2011: thick disk can form by radial migration; migrators increase their height above the plane
...
\caption{Metallicity range (16--84 percentile) of star particles as a function of their age for three structural components (defined as in Figures 6 - 7) identified at the solar circle of the Latte simulation: disk (red), in-situ halo (light blue) and accreted halo (dark blue).
